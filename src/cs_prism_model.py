...
class PrismModel:
    ...
    def write_prism_file(self, filename, init_b, init_ch):
        ...
        with open(path, 'w') as f:
            ...
            f.write('exe:[0..1] init 0;\n')
            f.write('success:[0..1] init 0;\n')
            ...
            f.write("[execute] (t<{0}) & (e=1) & (task_present=1) & (exe=0) ->0.9:(success'=1)&(exe'=1) + 0.1:(success'=0)&(exe'=1);\n".format(self.time_int))
            for action in self.actions:
                if action == 'gather_reward':
                    f.write("[{0}] (t<{1}) & (o=1) & (task_present=1) & (e=1) & (exe=1)-> (t'=t+1) & (o'=0) & (e'=0) & (exe'=0) ;\n".format(action, self.time_int))
                else:
                    f.write("[{0}] (t<{1}) & (o=1) & (task_present=1) & (e=1) & (exe=0)-> (t'=t+1) & (o'=0) & (e'=0) & (exe'=0) ;\n".format(action, self.time_int))
            ...
            for e,cl in enumerate(self.clusters):
                f.write('[gather_reward] (cl={0}) & (success=1):{1};\n'.format(e,cl))
            f.write('[gather_reward] (success=0):0;\n')
            f.write('\nendrewards\n\n')
...
if __name__ == '__main__':
    ...
    task_prob, task_prob_count, prob, rew_prob_count, clusters = ur.get_probabilistic_reward_model()
