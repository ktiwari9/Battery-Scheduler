    ...
    def __init__(self, battery_str):
        ...
        if len(arr) == 6:
            if ' ' != arr[0] and ' ' != arr[2] and ' ' != arr[4]:
                ...
                self.is_charging = float(arr[4].strip())
            ...
            if ' ' != arr[0] and ' ' != arr[6] and ' ' != arr[8]:   
                ...
                self.is_charging = float(arr[8].strip()) 
                self.date_time = datetime.fromtimestamp(self.epoch)
...
class BatteryModel:
    ...
    def extract_data(self, files):
        ...
        for file_set in files:
            ...
            # print (battery_data)
            if not battery_data.empty and battery_data.shape[0]>29:
                for i in range(battery_data.shape[0]-31):
                    if not math.isnan(battery_data.iloc[i,1]) and not math.isnan(battery_data.iloc[i+30,1]):
                        if battery_data.iloc[i,1] == 0 and battery_data.iloc[i+30,1] == 1:
                            c_b = int(round(battery_data.iloc[i,0]))
                            n_b = int(round(battery_data.iloc[i+30,0]))
                            if n_b not in self.gocharge_model[c_b]:
                                self.gocharge_model[c_b].update({n_b:1})
                            else:
                                self.gocharge_model[c_b][n_b] += 1
    ...
    def get_battery_model(self, paths):
        ...
        self.gocharge_model = dict()
        for model in [self.charge_model, self.discharge_model, self.gocharge_model]:
            for i in range (101):
                model.update({ i : dict()})
        ...
        with open(path+'/models/battery_discharge_model_new.yaml', 'w') as f_discharge:
            yaml.dump(self.discharge_model, f_discharge)
        with open(path+'/models/battery_gocharge_model_new.yaml', 'w') as f_gocharge:
            yaml.dump(self.gocharge_model, f_gocharge)
        with open(path+ '/models/battery_charge_model_new.yaml', 'w') as f_charge:
            yaml.dump(self.charge_model, f_charge)
        print ('Battery Models Created at: '+path+'/models/battery_discharge_model_new.yaml'+ ', '+ path+'/models/battery_charge_model_new.yaml')
