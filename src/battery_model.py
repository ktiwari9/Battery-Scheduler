...
def get_battery_model():
    ...
    path = roslib.packages.get_pkg_dir('battery_scheduler_sim')
    if os.path.isfile(path+'/models/battery_charge_model.yaml') and os.path.isfile(path+'/models/battery_discharge_model.yaml'):
        ...
        charging_data, discharging_data = extract_data(path+'/data/battery_data/') ## specify path to battery files
