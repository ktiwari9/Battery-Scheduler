...
class FiniteHorizonControl:
    ...
    def __init__(self, init_battery, init_charging, test_days, plan_id):
        ...
        self.path_rew = self.main_path + '/data/bc_pareto_sample_rewards'
            ...
            if self.pareto_points:
               ...
               for e, x in enumerate(self.pareto_points_sorted):
                    ...
                    if f_no != None:
                        ...
                        print 'Reading from bc_model_t'+f_no+'.adv'
                        self.pp = bc_read_adversary.ParseAdversary(['bc_model_t'+f_no+'.adv', 'bc_model_t.sta', 'bc_model_t.lab'])
                    else:
                    ...
                    self.get_plan('bc_pareto'+str(e)+'_'+self.plan_id+'_70b_'+str(rand_int), pareto_point)
    ...
    def simulate_day(self, day):
        ...
        for i in range(self.no_int):
            ...
            actions = []
            while not(('gather_reward' in actions) or ('go_charge' in actions) or ('stay_charging' in actions) or ('dead' in actions)):
                ...
                elif all(a == 'dead' for a in actions):
                    for s in nx_s:
                        t, tp, o, e, b, ch, cl = self.pp.get_state(s)
                        current_state = s
    ...
    def obtain_prism_model(self):
        pm = bc_prism_model.PrismModel('bc_model_t.prism', self.init_battery, self.init_charging, self.task_prob, self.clusters, self.prob, self.charge_model, self.discharge_model)
        
        #######################SPECIFY LOCATION ######################
        ### running prism and saving output from prism
        with open(self.path_data+'result_bc_fhc', 'w') as file:
            process = subprocess.Popen('./prism '+ self.path_mod + 'bc_model_t.prism '+ self.path_mod +'batterycost_model_prop.props -multimaxpoints 100 -v -exportadv '+ self.path_mod+ 'bc_model_t.adv -exportprodstates ' + self.path_mod +'bc_model_t.sta -exporttarget '+self.path_mod+'bc_model_t.lab',cwd='/home/milan/prism/prism/bin', shell=True, stdout=subprocess.PIPE)
        ...
        policy_file = []
        with open(self.path_data+'result_bc_fhc', 'r') as f:
            ...
            for line in line_list:
                if ': New point is (' in line:
                    ...
                    self.f_no_list.append(str(int(el[0][:-1])-2))
        
        # self.prob30_diff_list_sorted = sorted(self.prob30_diff_list)
        # print self.prob30_diff_list
        self.pareto_points_sorted = sorted(self.pareto_points)
        print self.pareto_points_sorted
...
if __name__ == '__main__':
    ...
    path = '/home/milan/workspace/strands_ws/src/battery_scheduler/data/bc_pareto_sample_rewards'
