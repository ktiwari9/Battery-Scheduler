...
class FiniteHorizonControl:
            ...
            if self.pareto_points:
               ...
               for e, x in enumerate(self.pareto_points_sorted):
                    ...
                    if f_no != None:
                        ...
                        self.pp = bc_read_adversary.ParseAdversary(['bcth_model_t'+f_no+'.adv', 'bcth_model_t.sta', 'bcth_model_t.lab'])
                    else:
                    ...
                    self.get_plan('bcth_pareto'+str(e)+'_'+self.plan_id+'_70b_'+str(rand_int), pareto_point)
    ...
    def obtain_prism_model(self):
        pm = bcth_prism_model.PrismModel('bcth_model_t.prism', self.init_battery, self.init_charging, self.task_prob, self.clusters, self.prob, self.charge_model, self.discharge_model)
        
        #######################SPECIFY LOCATION ######################
        ### running prism and saving output from prism
        with open(self.path_data+'result_bcth_fhc', 'w') as file:
        ...
        policy_file = []
        with open(self.path_data+'result_bcth_fhc', 'r') as f:
