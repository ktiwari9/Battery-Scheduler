...
class TaskBasedRBC2:  # Rule - charge only when battery goes below 40
    ...
    def simulate(self, init_battery, init_charging):
        ...
        for e, ts in enumerate(unique_ts):
            ...
            if battery <= 40 or (current_rew <= self.threshold and not bool(charging)):
                action = 'go_charge' 
                if battery <= 40:
                    replenish = True
...
if __name__ == '__main__':
    ...
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,7), datetime(2019,11,8), datetime(2019,11,9)])
    tbrbc2.get_plan('tbrbc2_711811911_1')

    np.random.seed(1)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,7), datetime(2019,11,8), datetime(2019,11,9)])
    tbrbc2.get_plan('tbrbc2_711811911_2')

    np.random.seed(2)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,7), datetime(2019,11,8), datetime(2019,11,9)])
    tbrbc2.get_plan('tbrbc2_711811911_3')

    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,10), datetime(2019,11,11), datetime(2019,11,12)])
    tbrbc2.get_plan('tbrbc2_101111111211_1')

    np.random.seed(1)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,10), datetime(2019,11,11), datetime(2019,11,12)])
    tbrbc2.get_plan('tbrbc2_101111111211_2')

    np.random.seed(2)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,10), datetime(2019,11,11), datetime(2019,11,12)])
    tbrbc2.get_plan('tbrbc2_101111111211_3')

    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,13), datetime(2019,11,14), datetime(2019,11,15)])
    tbrbc2.get_plan('tbrbc2_131114111511_1')

    np.random.seed(1)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,13), datetime(2019,11,14), datetime(2019,11,15)])
    tbrbc2.get_plan('tbrbc2_131114111511_2')

    np.random.seed(2)
    tbrbc2 = TaskBasedRBC2(70, 1, threshold, [datetime(2019,11,13), datetime(2019,11,14), datetime(2019,11,15)])
    tbrbc2.get_plan('tbrbc2_131114111511_3')