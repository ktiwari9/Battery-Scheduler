...
class RuleBasedControl:
	...
	def __init__(self, init_battery, init_charging):
		...
		self.test_rewards = []
		with open('/home/milan/workspace/strands_ws/src/battery_scheduler/data/rbct3_sample_rewards', 'r') as f:
			for line in f:
	...
	def simulate(self):
		...
		for i,r in enumerate(self.test_rewards):
			...
			elif (self.current_battery > 30 and self.current_battery < 70 and self.current_charging == 1) or (self.current_charging ==1 and r == 0):
				self.action.append('stay_charging')
				self.obtained_reward.append(0)
				nb = []
				p = []
...
if __name__ == '__main__':
	...
	path = '/home/milan/workspace/strands_ws/src/battery_scheduler/data/rbct3_sample_rewards'
	...
	np.random.seed(0)
	f_name = 'rbct3_oct123_70b_1'
	rbct3 = RuleBasedControl(70,1)

	rbct3.simulate()
	rbct3.get_plan(f_name)
	rewards_obtained = rbct3.get_rewards_from_plan(f_name)
	no_days = len(rbct3.test_rewards)/no_int
