...
class SampleGenerator:
    ...
    def __init__(self, test_days):
        ...
        #     for event in tasks: 
                # start = datetime.combine(anchor_day.date(), (datetime.utcfromtimestamp(event['start_after']['secs']) + timedelta(hours=5,minutes=30)).time())
                # end = datetime.combine(anchor_day.date(), (datetime.utcfromtimestamp(event['end_before']['secs']) + + timedelta(hours=5,minutes=30)).time())
        #         priority = event['priority']
        #         # priority = 1
...
if __name__ == "__main__":
    test_days = [datetime(2017, 10, 10), datetime(2017, 10, 19), datetime(2017,10,21)]
