...
if __name__ == '__main__':
    ...
    t1 = time.time()
    output_path = main_path + '/data/rhc_aug7'
    model_path = main_path + '/models/'
    with open(output_path, 'w') as fw:
        fw.write('time charging battery action sample_reward actual_reward exp_reward\n')
        for t in range(48*3):     # 48*3 for 3 days
            ...
            if next_state[2] == 'gather_reward':
                ...
                reward.append(0)
            fw.write('{0} {1} {2} {3}\n'.format(next_state[1], round(next_state[3]), actual_reward[t], exp_reward[t])) 
            init_battery = int(next_state[0][1])
            init_charging = int(next_state[0][0])
